(function() {
    ...
    angular.module("app.employee").factory("employeeLicensingService", employeeLicensingExportService);

    /** @ngInject **/
    function employeeLicensingExportService($rootScope, $http, $q, appUtils, DataUtils, employeeService, departmentSevice) {
        var services = {
        ...
        function _sheetsData(rawData, opts) {
            ...
            // cash Header
            var header = _.map(allSheetStruct.basicG.fields, function(cellS) {
                return [cellS.label, null, cellS.hStyle];
            });
            sheets.all.data.push(header);
            sheets.all.cols = _.map(allSheetStruct.basicG.fields, function(cellS) {
            ...
            _.each(rawData, function(item, index) {
                ...
                // end format Data
                // all summary Sheet
                var allSheetRow = _.map(allSheetStruct.basicG.fields, function(cellS) {
                    var val = item[cellS.field] || '',
                        cellData;
                    cellData = [val, null, cellS.style];
                    return cellData;
                });
                sheets.all.data.push(allSheetRow);

                // var availabilityH = _.map(allSheetStruct.availabilityG.fields, function(cellS, index) {
                //     if (index === 3) {
                //         return ['Availability', null, cellS.mainHStyle];
                //     }
                //     return ['', null, cellS.mainHStyle];
                // });
                // sheets.all.data.push(availabilityH);

                // var availabilityG = _.map(allSheetStruct.availabilityG.fields, function(cellS) {
                //     return [cellS.label, null, cellS.hStyle];
                // });
                // sheets.all.data.push(availabilityG);

                // var allAvailabilityG = _.map(allSheetStruct.availabilityG.fields, function(cellS) {
                //     var value = item.availability && item.availability[cellS.field] || 0,
                //         text = appUtils.availabilityEnum[value];
                //     if (_.trim(cellS.label) === '') {
                //         text = '';
                //     }
                //     var cellSubData = [text, null, cellS.style];
                //     return cellSubData;
                // });
                ...
                if (item.licenses) {
                    ...
                    sheets.all.data.push(licenseH);
                    var licenseG = _.map(allSheetStruct.licenseG.fields, function(cellS) {
                        return [cellS.label, null, cellS.hStyle];
                    });
